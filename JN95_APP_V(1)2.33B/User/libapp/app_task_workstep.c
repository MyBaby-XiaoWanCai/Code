...
uint8_t active_run_channel_numb = 1;		// 每个通道实际动作的模块数
...
int16_t BatteryFormattingProcess(uint8_t chx, uint8_t dir, uint8_t vmod, float v_target, float i_target)
{
...
#else
    if(i_target > 80.0f)	// 设定电流大于启动电流时所有通道工作
#endif
	{
        ch_run = ch_cnt;
    }
	...
	active_run_channel_numb = ch_run;
    ...
    for(j = 0; j < ch_run; j++)
    {
		GPIO_PinState Pin_Check = GPIO_PIN_RESET;
		
        Chg_Dis_Port_Cfg(Channel_Port_Table[chx_id + j], PORT_OUT_ENABLE);
		
		osDelay(10);
		
		Pin_Check = Chg_Dis_Port_Check(Channel_Port_Table[chx_id + j], PORT_OUT_ENABLE);
    }
...
/**********************************************************************************************************
**  函 数 名 : LM5170_Restart_Func()
**  功能说明 :
**  形    参 :
**  返 回 值 :
**********************************************************************************************************/
uint8_t LM5170_Restart_Func(uint8_t chx)
{
	uint8_t j = chx;
	...
	if(Tier2.workstepAnalyze[j].StepRunCur <= 5.0f)		// 输出电流设定值小于5A时不执行该操作
	{
		return 0;
	}
	
	osDelay(time_delay_tmp);
	
#ifdef YCJ			
	float Iout_tmp_1 = 0.0f;
	if(Mod_DIR_Flag[j] == BUCK_MODE)
	{
		Iout_tmp_1 = (float)((Tier2.calibrate[j].IhK_C * 2 * ad7175_value[j + 8]) / 16777216) + Tier2.calibrate[j].IhB_C - Tier2.calibrate[j].IhK_C;
	}
	else if(Mod_DIR_Flag[j] == BOOST_MODE)
	{
		Iout_tmp_1 = (float)((Tier2.calibrate[j].IhK_D * 2 * ad7175_value[j + 8]) / 16777216) + Tier2.calibrate[j].IhB_D - Tier2.calibrate[j].IhK_D;
	}
...
#else
	if(Tier2.workstepAnalyze[j].StepRunCur > 80.0f)
	{	
		float Iout_tmp_1 = 0.0f;
		if(Mod_DIR_Flag[2 * j + 0] == BUCK_MODE)
		{
			Iout_tmp_1 = (float)((Tier2.calibrate[2 * j + 0].IhK_C * 2 * ad7175_value[2 * j + 0 + 8]) / 16777216) + Tier2.calibrate[2 * j + 0].IhB_C - Tier2.calibrate[2 * j + 0].IhK_C;
		}
		else if(Mod_DIR_Flag[2 * j + 0] == BOOST_MODE)
		{
			Iout_tmp_1 = (float)((Tier2.calibrate[2 * j + 0].IhK_D * 2 * ad7175_value[2 * j + 0 + 8]) / 16777216) + Tier2.calibrate[2 * j + 0].IhB_D - Tier2.calibrate[2 * j + 0].IhK_D;
		}
		
		float Iout_tmp_2 = 0.0f;
		if(Mod_DIR_Flag[2 * j + 1] == BUCK_MODE)
		{
			Iout_tmp_2 = (float)((Tier2.calibrate[2 * j + 1].IhK_C * 2 * ad7175_value[2 * j + 1 + 8]) / 16777216) + Tier2.calibrate[2 * j + 1].IhB_C - Tier2.calibrate[2 * j + 1].IhK_C;
		}
		else if(Mod_DIR_Flag[2 * j + 1] == BOOST_MODE)
		{
			Iout_tmp_2 = (float)((Tier2.calibrate[2 * j + 1].IhK_D * 2 * ad7175_value[2 * j + 1 + 8]) / 16777216) + Tier2.calibrate[2 * j + 1].IhB_D - Tier2.calibrate[2 * j + 1].IhK_D;
		}

		if((Iout_tmp_1 < 0.1f) || (Iout_tmp_2 < 0.1f))
		{
			return 2;
		}
	}
...
//////			osDelay(200);
//////			uint8_t lm5170_restart_flag = LM5170_Restart_Func(j);
//////			if(lm5170_restart_flag == 2)
//////			{
//////				BatteryFormattingStop(j);
//////				osDelay(20);
//////				BatteryFormattingProcess(j, BUCK_MODE, V_SENSE, Tier2.workstepAnalyze[j].StepRunVol, Tier2.workstepAnalyze[j].StepRunCur);			
...
void CCtoCVConvertFunc(void)
{
	if(g_ucSysMode == SYS_NORMAL_MODE)
	{
		for(int j = 0; j < CHANNEL_CFG; j++)    // 判断每个通道是否更新控制命令
		{
...
void AppTaskWorkstep(void *argument)
{
    ...
    while(1)
    {
        ...
        if(test_printf_workstep < 20)
        {
            test_printf_workstep++;
        }
		else
		{
			test_printf_workstep = 0;
