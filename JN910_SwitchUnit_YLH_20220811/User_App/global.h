...
typedef struct
{
	...
	s_Tier1_ProtectStruct AmpVdrop;			// 6. 电压快速下降阈值
	s_Tier1_ProtectStruct CntVdrop;			// 7. 电压快速下降次数
	s_Tier1_ProtectStruct Vovsense;			// 8. 硬件过压保护门限
	...
	s_Tier1_ProtectStruct VauxThr;			//16. 辅助电压异常门限
    s_Tier1_ProtectStruct SNERR;			//17. 电压异常误差门限
...
typedef struct __attribute__ ((__packed__))
{
	...
	s_WorkStepSegmentStruct RunSeg[RUN_SEG_NUM];	// 工步运行类型
	s_WorkStepSegmentStruct AbtSeg[ABT_SEG_NUM];	// 工步截止类型
	s_WorkStepSegmentStruct ProSeg[PRO_SEG_NUM];	// 工步保护类型	
...
typedef struct __attribute__ ((__packed__))
{
	...
	uint16_t Datelen;						// 数据长度 0x09
	uint8_t  FeatureCode;					// 特征码
	uint8_t  StateCode;						// 状态码 0x00
	uint8_t  ChanelFlag;					// 通道码
...
typedef struct {
	...
	__IO uint8_t processedFlg;				// 回复处理标志: 0-已处理、1-待处理  
}ACDC_UpdateStrcut;   

/* 全局保护触发类型枚举 */
enum GlobalProtect_state
{
    GlobalProtect_OV   = 0x01 << 0,			// 1.过压保护
    GlobalProtect_UV   = 0x01 << 1,			// 2.欠压保护
    GlobalProtect_COA  = 0x01 << 2,			// 3.充电过流保护
    GlobalProtect_DOA  = 0x01 << 3,			// 4.放电过流保护
    GlobalProtect_OC   = 0x01 << 4,			// 5.过容保护
    GlobalProtect_MV   = 0x01 << 5,			// 6.初始电压上限保护
    GlobalProtect_LV   = 0x01 << 6,			// 7.初始电压下限保护
    IntelProtect_NONE1 = 0x01 << 7,			// 8.未定义
    IntelProtect_NONE2 = 0x01 << 8,			// 9.未定义
    IntelProtect_VCV   = 0x01 << 9,			//10.接触检测电压变化量
    IntelProtect_OTE   = 0x01 << 10,		//11.其他故障
    IntelProtect_INV   = 0x01 << 11,		//12.无效 
};
...
extern Aux_Record_Struct aux_record[8];
