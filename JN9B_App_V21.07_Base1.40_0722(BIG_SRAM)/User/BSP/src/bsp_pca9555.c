...
uint8_t PB_Addr_Arr[8] = {PB_CH1_ADDR,PB_CH2_ADDR,PB_CH3_ADDR,PB_CH4_ADDR,PB_CH5_ADDR,PB_CH6_ADDR,PB_CH7_ADDR,PB_CH8_ADDR};	//新地板对应功率板地址
...
StatusType PCA_9548WRTo_9555(I2C_DevType DevNum,uint8_t CHx,uint8_t Mode,uint8_t *WriteData,uint8_t State)
{
    ...
    for(uint8_t cnt = 0; cnt < 20; cnt++)
    {
		...
		{
				if(Pca9555_RegRead(DevNum,PB_Addr_Arr[CHx - 1],Pca9555_Data.Power_Board[CHx - 1].ReadData) == _ERR)
				{
...
//			if(vers_mana.Bom_Vers == C042B_BOOM_VER)
		{
				if(Pca9555_RegRead(DevNum,PCA9555_POWER_ADDR,Pca9555_Data.Power_Board[CHx - 1].ReadData) == _ERR)
				{
//					//读取当前所选通道各IO状态失败、返回错误状态
				}
		}
		...
		if(Mode == PCA_WRITE)	//写操作
		{
				...
				if(Bom_Vers == C042C_BOOM_VER)
				{
						Res = Pca9555_RegWrite(DevNum,PB_Addr_Arr[CHx - 1],WriteData);	//一次写两个字节
				}
				else
//						if(vers_mana.Bom_Vers == C042B_BOOM_VER)
				{
						Res = Pca9555_RegWrite(DevNum,PCA9555_POWER_ADDR,WriteData);	//一次写两个字节
				}
		}
...
StatusType Pca9555_WRTo_M_Loop(I2C_DevType DevNum,uint8_t CHx,uint8_t State,uint8_t *WriteData)
{
    ...
    if(Bom_Vers == C042C_BOOM_VER)
    {
        if(Pca9555_RegRead(DevNum,PB_Addr_Arr[CHx],Pca9555_Data.Power_Board[CHx].ReadData) == _ERR)
        {
            //读取当前所选通道各IO状态失败、返回错误状态
        }
    }
...
//		if(vers_mana.Bom_Vers == C042B_BOOM_VER)
    {
        if(Pca9555_RegRead(DevNum,PCA9555_POWER_ADDR,Pca9555_Data.Power_Board[CHx].ReadData) == _ERR)
        {
            //读取当前所选通道各IO状态失败、返回错误状态
        }
    }
    ...
    if(State == 0x01)	//写1
    {
        WriteData[0] =  Pca9555_Data.Power_Board[CHx].ReadData[0] | WriteData[0]; // & 0xFF 保持该八位数据不变
        WriteData[1] =  Pca9555_Data.Power_Board[CHx].ReadData[1] & WriteData[1]; //对选中位置高
    }
    else if(State == 0x00) //写0
    {
        WriteData[0] =  Pca9555_Data.Power_Board[CHx].ReadData[0] & WriteData[0];
        WriteData[1] =  Pca9555_Data.Power_Board[CHx].ReadData[1] & WriteData[1]; //选中位置低
    }

    if(Bom_Vers == C042C_BOOM_VER)
    {
        Res = Pca9555_RegWrite(DevNum,PB_Addr_Arr[CHx],WriteData);
    }
    else
//		if(vers_mana.Bom_Vers == C042B_BOOM_VER)
    {
        Res = Pca9555_RegWrite(DevNum,PCA9555_POWER_ADDR,WriteData);
    }
