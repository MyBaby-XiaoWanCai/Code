...
//			Tier2ToTier1(j, SYNC_TYPE_GPROT);
			...
			Dis_int();
			ls = osKernelLock(); 
			ee_WriteBytes(ucEepBuf, PROTECT_GLOBAL_BASEADDR(j), sizeof(PROTECT_GLOBAL_STRUCT(j)));			
			us =osKernelUnlock();
...
//			Tier2ToTier1(j, SYNC_TYPE_IPROT);
			...
			Dis_int();
			ls = osKernelLock(); 
			ee_WriteBytes(ucEepBuf, PROTECT_INTERNAL_BASEADDR(j), sizeof(PROTECT_INTERNAL_STRUCT(j)));		
			us =osKernelUnlock();
			osKernelRestoreLock(us);
			osKernelRestoreLock(ls);
			En_int();
...
//			Tier2ToTier1(j, SYNC_TYPE_CALI);
            ...
            Dis_int();
            ls = osKernelLock();             
			ee_WriteBytes(ucEepBuf, CALIBRATE_BASEADDR(j), sizeof(CALIBRATE_STRUCT(j)));		
			us =osKernelUnlock();
			osKernelRestoreLock(us);
			osKernelRestoreLock(ls);
			En_int();
...
void CalibrateDatInit(uint8_t chx)
{
	...
	Tier2.calibrate[j].IhSetK_C     = 1338;        //修改1
	...
	Tier2.calibrate[j].IhSetK_D     = 1338;        //修改1
