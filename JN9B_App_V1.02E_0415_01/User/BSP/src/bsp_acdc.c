...
//		stcInit.u8TransMode = CAN_TRANS_PTB_STB_SSHOT;
...
static void CAN2_IrqCallback(void)
{
	...
	CAN2_ResvData();                                          //CAN1接收处理
	CAN_ClrStatus(APP_CAN2_UNIT, m_u32StatusVal);              //清除CAN2状态标志
}
...
static void Can2IrqConfig(void)
{
	...
	stcCfg.pfnCallback = &CAN1_IrqCallback;                    //设定中断回调函数
