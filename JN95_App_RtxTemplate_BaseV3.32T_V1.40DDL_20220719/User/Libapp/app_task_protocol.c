...
static uint16_t Pack_NO = 0;	//网口升级使用的升级包序号
static uint32_t Write_ADDR = EFM_ADDR_SECTOR128;
...
{
    uint8_t result = 0;			/* 解析返回值 */
    uint8_t scmd = 0;			/* 子命令码 */
    uint8_t scmd_offset = 0;	/* 子命令码偏移 */
    float   sdat = 0.0f;		/* 子命令码数据 */	/* 设置指令子命令码为数据 */
...
void AuxRecordDatUpcast(void)
{
	...
	for(uint8_t i = 0; i < 7; i++)
	{
		if(aux_record_flag & (0x01 << i))
		{
			...
			W5500_send(0, aux_record_temp, 51);
    ...
    if(step_type != SCMD_WORKSTEP_TYPE_FINISH)		/* 非结束工步 */
    {
        for(int j = 0; j < CHANNEL_CFG; j++)
        {
            ...
            if(unpack.chEn & umask)
            {
                ...
                {
                    if( ((p + i)->type != SCMD_INVALID) && (((p + i)->type <= SCMD_WORKSTEP_PARAM_BEGIN) || ((p + i)->type >= SCMD_WORKSTEP_PARAM_END)) )
                    {
                        result = FRAME_SCMD_ERR;
...
//					uint8_t abort_priority  = (abort_condition >> 4) & 0x0F;
                    ...
                    if((step_type == SCMD_WORKSTEP_TYPE_CC ) || (step_type == SCMD_WORKSTEP_TYPE_DCC))
                    {
                        if(abort_type == SCMD_WORKSTEP_PARAM_VOLTAGE)
                        {
                            data_check = 2;
                        }
                        else if((abort_type != SCMD_WORKSTEP_PARAM_TIME) && (abort_type != SCMD_WORKSTEP_PARAM_CAPACITY))
                        {
                            result = FRAME_SCMD_ERR;
                            scmd_reply = 3;			/* 工步设定类型不匹配(跳出) */
                            memcpy(pSend + *lenSend, &step_numb , 2);
                    ...
                    {
                        if(abort_type == SCMD_WORKSTEP_PARAM_CURRENT )
                        {
                            data_check = 2;
                        }
                        else if((abort_type != SCMD_WORKSTEP_PARAM_TIME) && (abort_type != SCMD_WORKSTEP_PARAM_CAPACITY))
...
//				EEP_DAT_SYNC(CALIBRATE_ID(j));
                ...
                ee_ReadBytes((uint8_t *)&CALIBRATE_STRUCT(j), CALIBRATE_BASEADDR(j), sizeof(CALIBRATE_STRUCT(j)));
                memcpy(&Tier2.calibrate[j], &Tier1.calibrate[j], sizeof(Tier1.calibrate[j]));
            }
        }
    }
    ...
    else
    {
        ...
        case 0x01:                /* 跳转boot模式 */
            if(System_UpdateFlg==0)  //升级过程中不容许执行2次
            {
                ...
                NVIC_DisableIRQ(APP_CAN1_IRQn);             //关闭CAN中断
        ...
        case 0x04:                   /*写升级区数据，一次1024字节*/
            if(lenRecv==1037)      //检查数据长度是否等于升级数据包固定长度9+1024+4
            {
                Rx_Pack_NO=(pRecv[8]<<8)+pRecv[7]; //记录接收包序号
                if(Rx_Pack_NO == Pack_NO)
                {
                    memcpy(updatePack,&pRecv[9],1024);  //拷贝升级数据到缓存
                    __set_PRIMASK(1);                    //关中断
    ...
    *lenSend += 1;
    ...
    case 0x02:  /* 透传数据 */
    {
        if((ACDC_UpdateFlg == 1) && ((ACDC.Type == 2) || (ACDC.Type == 22))) //普德模块升级
        {
            ...
            CANx_SendData(APP_CAN1_UNIT, ID, Data, 8);         //转发can
            ...
            else if(((ID >> 24) & 0xFF) == 0x1c && ((ID >> 16) & 0xF0) == 0xa0)
            {
                w5500_Silence = 0;
            ...
            else if(((ID >> 24) & 0xFF) == 0x1c && ((ID >> 16) & 0xF0) == 0xd0)
            {
                w5500_Silence = 0;
...
void Buffer_Data_To_Record(void)
{
	...
	{
		W5500_send(0, Record.Msg[addr].Dat, Record.Msg[addr].Len);
        ...
        if(osSemaphoreAcquire(w5500_rcv_Semaphore, osWaitForever) == osOK) //等待同步信号量
        {
            ...
            while(unpack_len < w5500_rcvbuf.rcv_len)		//判断数据是否解析完毕
            {
                ...
                else											//符合最短完整帧长度要求(≥10个字节)
                {
                    ...
                    if(!w5500_Silence)
                    {
                        while(SOCK_BUSY == W5500_send(SOCK_0_NUMB, ucNetSendBuf, usNetSendCnt));			//根据解析结果回复网口上位机
                        if(ACDC_UpdateFlg == 2)		//ACDC升级完成
                        {
                            bsp_DelayMS(5000);		//等待5秒
                            NVIC_SystemReset();		//重启
