...
void Socket_Receive(void)
{
    if((!(TCP_SERVER_FLAG & 0x40))&&(TCP_Socket[SERV_TCP_WORK].Data_RX.Status_Recive==0x01)) {
        ...
        memcpy(tcp_recvbuf, TCP_Socket[SERV_TCP_WORK].Data_RX.Data, TCP_Socket[SERV_TCP_WORK].Data_RX.Num_Recived);//TCP_Socket[SERV_TCP_WORK].Data_RX.Num_Recived)
//         for(int i = 0; i < W5500_Service.Ptr->Data_RX.Num_Recived; i++) {
//             tcp_recvbuf[i] = W5500_Service.Ptr->Data_RX.Data[i];
//         }
       // Socket_Sent(2);
