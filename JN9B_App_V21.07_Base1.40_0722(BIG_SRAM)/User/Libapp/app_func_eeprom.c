...
#ifdef	YCJ	
  Tier2.gProt[j].Iovcharge.value    = (132 * (CHANNEL_MAX / CHANNEL_CFG));	//充电过流门限 32
	Tier2.gProt[j].Iovdischg.value    = (132 * (CHANNEL_MAX / CHANNEL_CFG));	//放电过流门限 32
#else    
...
#ifdef	YCJ	
	Tier2.iProt[j].IoutThr.value   = 132*(CHANNEL_MAX / CHANNEL_CFG);	//YCJ 电流上限
#else    
	Tier2.iProt[j].IoutThr.value   = 168;
#endif

	Tier2.iProt[j].VbusThrH.value  = 17;
	if(CHANNEL_USE == 0x04)	//4通道机型
	{
		Tier2.iProt[j].VbusThrL.value  = 11;
	}
	else
	{
		Tier2.iProt[j].VbusThrL.value  = 12;	
	}
	Tier2.iProt[j].IccDiff.value   = 0.32*(CHANNEL_MAX / CHANNEL_CFG);	
