...
**                                           函数声明区
**********************************************************************************************************/
void CAN_SendMultiFrames(uint32_t id, uint8_t* pData, uint16_t len);
...
void Can_Data_Pack_Send(u32_t feature_id, u32_t id_type, u8_t *tx_dat, u16_t tx_len)
{
	...
	{
		CAN_SendMultiFrames(feature_id, tx_dat, tx_len);
	}
...
extern uint8_t CAN1_SendData(uint32_t ID, uint8_t *pData, uint16_t len);
...
void CAN_SendMultiFrames(uint32_t id, uint8_t* pData, uint16_t len)
{
	...
	while(lenTemp)
	{
		...
		if(lenTemp > 8)
		{
//			while(1 != CAN1_SendData(idTemp, p, 8));
			CAN1_SendData(idTemp, p, 8);
			lenTemp -= 8;
			p       += 8;			
		}
		else
		{
//			while(1 != CAN1_SendData(idTemp, p, lenTemp));
			CAN1_SendData(idTemp, p, lenTemp);
			lenTemp = 0;
		}
	}
}
