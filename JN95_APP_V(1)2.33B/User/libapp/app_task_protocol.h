...
/* 8 BOOT升级*/
#define CMD_BOOT_UPDATE                     0x20        /* BOOT升级命令 */
/* 9 无效类 */
...
/*BOOT升级子命令*/
#define CMD_BOOT_UPDATE_ENTER               0x01        /* 进入BOOT升级模式 */
#define CMD_BOOT_UPDATE_EARSE               0x02        /* 擦除BOOT区FLASH */
#define CMD_BOOT_UPDATE_WRITE               0x03        /* 写入BOOT区FLASH */
#define CMD_BOOT_UPDATE_EXIT                0x04        /* 退出BOOT升级模式*/
...
#ifdef YCJ
/* 版本号 */
#define SOFTWARE_PRIVERSION					12.33f	/* 主版本号 */
#define SOFTWARE_SUBVERSION					'B'	    /* 子版本号 */
#else
/* 版本号(V2.33B) */
#define SOFTWARE_PRIVERSION					2.33f	/* 主版本号 */
#define SOFTWARE_SUBVERSION					'B'		/* 子版本号 */
#endif
...
typedef struct
{
	...
	uint8_t Eptr;
	uint8_t Idle;
	Record_Msg_Struct Msg[MSG_NUM];
} Record_Buffer_Struct;
...
extern uint16_t debug_ch_onoff_state;

extern uint8_t debug_w5500_mode;
extern uint8_t debug_w5500_mode_change;

extern uint8_t aux_record_flag;
...
extern void AuxRecordDatSync(uint8_t err_type);
extern void AuxRecordDatUpcast(void);
extern void RecordDatUpcast(uint8_t chx, uint8_t sync_flag);

extern void WorkstepProtUpcast(uint8_t chx);


extern void Record_Msg_Init(void);
extern void Record_Data_To_Buffer(uint8_t* dat, uint16_t len);
extern void Buffer_Data_To_Record(void);
