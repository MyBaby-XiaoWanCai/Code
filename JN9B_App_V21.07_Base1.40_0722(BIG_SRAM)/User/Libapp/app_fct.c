...
uint8_t FCTRun_State = 0x00;	//整机FCT运行状态标志 0-未运行,1-运行中,2-运行结束
/**********************************************************************************************************
**                                           函数声明区
**********************************************************************************************************/
...
while(StepTemp != FCT_END_NOP)
{
	switch(StepTemp)
	{
		...
		{
			memset(FCT_WriteBuf,0x00,64);	//FCT结果缓存数组清空
...
uint8_t WholeMachine_FCTVersion()
{
	...
	/* 控制板版本号判断 */
	if((Bom_Vers == BOM_VER) && (Id_Vers == ID_VER) && (Pcb_Vers == PCB_VER))	
	{
...
//		FCT_WriteBuf[10] = 0x01;	//版本号错误,向eeprom地址A+9写1
	...
	for(VerReadCnt = 1; VerReadCnt <= 8; VerReadCnt++)
	{
		...
		{		
			/* 功率板版本号判断 */
			if(Pow_Ver[VerReadCnt - 1] == PB_VER)	//功率板版本号判断
			{
...
//		if(IOTestInfo.MainPowerSignal[i + 1] == 0x00)	//在位信号判断 1-8 0位为主控板在位信号，0在位，1不在位
//		{
//			Chg_Dis_On_Cfg((1<<i), BUCK_MODE, V_SENSE, 4.5, 2.0);
//		}
//	}
//	osDelay(3000);	
//		
...
uint8_t WholeMachine_FCTVsenseSlowLoop()
{
	...
	for(i = 0;i < CHANNEL_USE;i++)
	{
		if(IOTestInfo.MainPowerSignal[i + 1] == 0x00)
		{
			Chg_Dis_On_Cfg((1<<i), BUCK_MODE, V_SENSE, 4.5, 2.0);
		}
		osDelay(3000);
...
uint8_t WholeMachine_FCTEnd()
{
	...
	static uint8_t FCT_Test_Num = 0x36;
...
//	}
	...
	for(Test_Cnt = 1; Test_Cnt < FCT_Test_Num; Test_Cnt++)	//NG项数
	{
		if(FCT_WriteBuf[Test_Cnt] != 0x00)
		{
			NG_Cnt +=1;
		}
	}
