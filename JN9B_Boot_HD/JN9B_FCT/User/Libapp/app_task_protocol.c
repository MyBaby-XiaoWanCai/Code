...
uint8_t ACDC_UpdateFlg = 0;           //ACDC升级标志，0禁止，1允许
    ...
    *lenSend += 1;
    ...
    switch(scmd)
    {
        ...
        case SCMD_SYSTEM_AC_Q:         /* AC侧电压查询 */
        {
            ...
            bsp_DelayMS(1000);     //延时1秒  等待can回复
            memcpy(pSend + *lenSend, (const float *)&ACDC.ACV[0], 4) ;    //填充回复数据长度段
            *lenSend += 4;
#else
            if(ACDC.Type == 1) //国电
            {
                ACDC_ACV_Query_CMD(ACDC.Type, 0); //查询模块0的AC电压
                ACDC_ACV_Query_CMD(ACDC.Type, 1); //查询模块1的AC电压
            }
                else
                    if(ACDC.Type == 2) //普德
                    {
                        ACDC_ACV_Query_CMD(ACDC.Type, 1); //查询模块1的AC电压
                        ACDC_ACV_Query_CMD(ACDC.Type, 2); //查询模块2的AC电压
                    }
            
            bsp_DelayMS(500);     //延时1秒  等待can回复
            memcpy(pSend + *lenSend, (const float *)&ACDC.ACV[0], 4) ;    //填充回复数据长度段
            *lenSend += 4;  
        ...
        case SCMD_SYSTEM_ACDC_VERSION_Q:           /* ACDC版本查询 */
        {
            ...
            *lenSend += 4;
            ...
            if(ACDC.Type == 1) //国电
            {
                ACDC_FW_VERSION_CMD(ACDC.Type, 0); //查询模块0的固件版本
                ACDC_FW_VERSION_CMD(ACDC.Type, 1); //查询模块1的固件版本
            }
                else
                    if(ACDC.Type == 2) //普德
                    {
                        ACDC_FW_VERSION_CMD(ACDC.Type, 1); //查询模块1的固件版本
                        ACDC_FW_VERSION_CMD(ACDC.Type, 2); //查询模块2的固件版本
                    }
	...
	else
	{
		switch(scmd)
		{
            ...
            case 0x11:      /* 设置铁电数据 */
            {
                ...
                MB85Ctrl_CheckWrite(Base_Add, pRecv + lenAnalysis, date_len); //设置数据写入铁电
            }
            break;
    ...
    *lenSend += 1;
    ...
    switch(scmd)
    {
        case 0x01:	/* 开启网口-CAN透传模式 */
		{
            ...
            NVIC_DisableIRQ(APP_TMRA1_IRQn);        //HAL_NVIC_DisableIRQ(TIM3_IRQn);         //关闭告警判断
            NVIC_DisableIRQ(APP_TMRA3_IRQn);         //关闭运行时计数
            osTimerStop(timerID_Periodic_adc3);     //关闭软定任务
...
//				memcpy(&ID, (const void *)(pRecv + lenAnalysis + 1), 4);          				//如果厂家ID按小端模式发送，不做小端转换
                ...
                CANx_SendData(APP_CAN_UNIT, ID, Data, 8);			//转发can
                
                /*等待can接收数据，处理打包*/
                if( (!(((ID >> 24) & 0xFF) == 0x1c && ((ID >> 16) & 0xF0) == 0xb0)) && \
					(!(((ID >> 24) & 0xFF) == 0x1c && ((ID >> 16) & 0xF0) == 0xa0)) )			//can数据报 ID以0x1c 0xbx开头的ACDC模块不回复,无需等待
                {
                    w5500_Silence = 0;
                ...
                {
					w5500_Silence = 0;
					while(TimOutCNT < 40000)		//握手包需要等待回复确认
					{
						TimOutCNT++;
						bsp_DelayUS(100);
...
//	float   sdat = 0.0f;		/* 子命令码数据 */
	...
	*lenSend += 1;	
	...
	if((Tier2.debug.set.Vtemp <= 7.0f) && (Tier2.debug.set.Itemp <= 95.0f))
    {
		...
		scmd_reply = 0x01;			/* 更新发送缓存区回复数据(ACK) */
    }
    else																		/* 设置参数非法 */
    {
        result     = FRAME_SCMD_ERR;
        scmd_reply = 0x00;			/* 更新发送缓存区回复数据(NAK) */
    }
...
//	uint8_t scmd_offset = 0;	/* 子命令码偏移 */
	...
	*lenSend += 1;		
    ...
    if((scmd >= SCMD_DEBUG_SYSTEM_SET_BEGIN) && (scmd <= SCMD_DEBUG_SYSTEM_SET_END))
    {
        ...
        scmd_reply = 0x01;			/* 更新发送缓存区子命令码数据(ACK) */
    }
    else
    {
        result     = FRAME_SCMD_ERR;
        scmd_reply = 0x00;			/* 更新发送缓存区子命令码数据(NAK) */
    }
