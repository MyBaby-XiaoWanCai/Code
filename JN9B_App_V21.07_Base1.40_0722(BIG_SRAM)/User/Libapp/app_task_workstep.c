		...
		while(current_change_done == 0)
		{
			...
			for(j = 0; j < ch_cnt; j++)
			{
				...
				else if(i_set[chx_id + j] > i_dac[chx_id + j])
				{
					...
					{
						i_set[chx_id + j] -= 6;//DECREASE_STEP;
					}
					else
					{
						i_set[chx_id + j] = i_dac[chx_id + j];
...
int16_t BattFormatPro_Debug(uint8_t chx, uint8_t dir, uint8_t vmod, float v_target, float i_target)
{
		uint8_t j = chx;
...
/**********************************************************************************************************
**  函 数 名 : Vbat_Pre_Check()
**  功能说明 : 启动工步流程前用于检测电池电压欠压故障的函数
**  形    参 : chx - 通道
**  返 回 值 : 0 - 成功，其他失败
**********************************************************************************************************/
uint8_t Vbat_Pre_Check(uint8_t _no)
{
	if(_no >= PORT_NO8)		// 内部通道号超出范围
	{
		return 1;
	}	
	
	/* 电池欠压检测 */
	if((Tier2.gProt[_no].VbatThrL.isEnable & 0x80) == 0)
	{
		if(Tier2.chRecord[_no].Vbat < Tier2.gProt[_no].VbatThrL.value)
		{
			GP_Alarm_Flag[_no] |= GlobalProtect_UV;
...
**  返 回 值 : 0 - 成功，其他失败
**********************************************************************************************************/
uint8_t mloop_flg[8] = {0x00};	//慢速环置位标志
...
//	if(CHANNEL_CFG == 0x08 && mloop_flg[chx] == 0x00)	//120机型
//	{
//		Enable_Mloop(chx);	//默认进入慢速环
//		
//		mloop_flg[chx] = 0x01;
//	}
	
	if(CHANNEL_CFG == 0x01)	//1CH 600A机型
	{	
		//同方向类型工步之间切换中间不停止 充电-充电 放电-放电，用于DCIR
		if( ((Tier2.workstepRunning[j].StepType == SCMD_WORKSTEP_TYPE_CC) || (Tier2.workstepRunning[j].StepType == SCMD_WORKSTEP_TYPE_CV))
		 && ((Tier2.workstepAnalyze[j].StepType == SCMD_WORKSTEP_TYPE_CC) || (Tier2.workstepAnalyze[j].StepType == SCMD_WORKSTEP_TYPE_CV)) )	//恒流/恒压 放
		{
		
		}
		else 
		if( ((Tier2.workstepRunning[j].StepType == SCMD_WORKSTEP_TYPE_DCC) || (Tier2.workstepRunning[j].StepType == SCMD_WORKSTEP_TYPE_DCV))
		 && ((Tier2.workstepAnalyze[j].StepType == SCMD_WORKSTEP_TYPE_DCC) || (Tier2.workstepAnalyze[j].StepType == SCMD_WORKSTEP_TYPE_DCV)) )	//恒流/恒压 充
		{
	...
	switch(Tier2.workstepAnalyze[j].StepType)
	{
		...
		{
			time_over_stop[j] = 0;
			uint8_t pre_check_flag = Vbat_Pre_Check(j);
			if(((Tier2.workstepAnalyze[j].StepAbtVol > Tier2.chRecord[j].Vbat) || (Tier2.workstepRunning[j].RunSta == WORKSTEP_RUNSTA_REF_CONTACT)) && (pre_check_flag == 0))
			{			
		...
		else if(Tier2.workstepAnalyze[j].StepType == SCMD_WORKSTEP_TYPE_DCC)
		{
			...
			else
			{
				Tier2.workstepAnalyze[j].StepRunVol = Tier2.workstepAnalyze[j].StepAbtVol - 0.1f;
				 ...
				 (Tier2.workstepAnalyze[j].StepType == SCMD_WORKSTEP_TYPE_DCV) )	//恒压充、放电模式
		{
			if(Tier2.workstepAnalyze[j].StepRunCur == 0)
			{
				Tier2.workstepAnalyze[j].StepRunCur = WORK_CUR_MAX;
...
int16_t Workstep_Switch_Func(uint8_t chx)
{
	...
	if(Tier2.workstepRunning[j].RunSta == WORKSTEP_RUNSTA_REF_RUNNING)	//工步运行中
	{
		if(Workstep_Reload_Func(j) == -1)	//工步参数加载
		{
//			RecordDatUpcast(j, 1);
			...
			BatteryFormattingStop(j);
			Tier2.workstepRunning[j].RunSta = WORKSTEP_RUNSTA_REF_WAIT;	//等待状态
			Tier2.workstepRunning[j].Trun = 0;
			run_time_cnt[j] = 0;
			return -1;
		}
...
//			RecordDatUpcast(j, 1);
	...
	if(Tier2.workstepRunning[j].RunSta == WORKSTEP_RUNSTA_REF_WAIT)	//等待状态
	{
	...
	if(Tier2.workstepRunning[j].StepType == SCMD_WORKSTEP_TYPE_FINISH)	//工步结束
	{
		BatteryFormattingStop(j);
...
void AppTaskWorkstep(void *argument)
{
	...
	while(1)
	{
		Workstep_Analysis_Func();	//工步参数解析

		Workstep_Abort_Func();	//工步停止

		CCtoCVConvertFunc();	//CC模式切换CV模式
		...
		for(int j = 0; j < CHANNEL_USE; j++)
		{
			if( ((debug_ch_onoff_state >> (2 * j)) & 0x03) == 1)
			{
				...
				if( ((CHANNEL_CFG == 0x04 && j <= 3) || (CHANNEL_CFG == 0x02 && j <= 1) || (CHANNEL_CFG == 0x01 && j <= 0)) 
				 && ParaCHxConfig != 0x01 ) //并机模式 - 关
				{
					Debug_Weaver_2(j,1);
				}
				else
...
/**********************************************************************************************************
**  函 数 名 : 通道并机Debug模式控制函数
**  功能说明 : CHx - 开启通道 State - 开关状态
**  形    参 : 
**  返 回 值 : 无
**********************************************************************************************************/
void Debug_Weaver_2(uint8_t CHx,uint8_t State)
{
	uint8_t CH_OnOff[4] = {0x03,0x0C,0x30,0xC0};
	
	if(State == 0x01)	//关机
	{
//		BatteryFormattingStop(CHx);
		
		Chg_Dis_Off_Cfg(CH_OnOff[CHx]);
	}
	else 
	if(State == 0x02)	//开机
	{
		...
		else if (CHANNEL_CFG == 0x02)	//4通道并机
		{
			GPIO_ResetPins(GPIO_PORT_CVK_4,GPIO_PIN_CVK_4);
			GPIO_SetPins(GPIO_PORT_CVK_8,GPIO_PIN_CVK_8);
			...
			for(uint8_t MS_j = 0;MS_j < 8;MS_j++)
			{
				if(MS_j == 0x00 || MS_j == 0x04)
					Chg_Dis_Port_Cfg(Channel_Port_Table[MS_j], PORT_MODE_MASTER);	//1、5通道设置为主机模式
				else
					Chg_Dis_Port_Cfg(Channel_Port_Table[MS_j], PORT_MODE_SLAVE);
			}
		}
		else if (CHANNEL_CFG == 0x01)	//8通道并机
		{
			GPIO_ResetPins(GPIO_PORT_CVK_4,GPIO_PIN_CVK_4);
			GPIO_ResetPins(GPIO_PORT_CVK_8,GPIO_PIN_CVK_8);
			...
			{
				if(MS_j == 0)
					Chg_Dis_Port_Cfg(Channel_Port_Table[MS_j], PORT_MODE_MASTER);	//1通道设置为主机模式
				else
					Chg_Dis_Port_Cfg(Channel_Port_Table[MS_j], PORT_MODE_SLAVE);
			}	
		}
		
		BattFormatPro_Debug(CHx, Tier2.debug.set.PortDir, Mod_V_Range_Flag[CHx], Tier2.debug.set.Vtemp, Tier2.debug.set.Itemp*(CHANNEL_MAX / CHANNEL_CFG));		
		
//		Chg_Dis_On_Cfg(CH_OnOff[CHx], Tier2.debug.set.PortDir, Mod_V_Range_Flag[CHx], Tier2.debug.set.Vtemp, Tier2.debug.set.Itemp);
	}
}
