...
{
////#ifdef C042A_7606	
	int32_t VsenseIns[8];					// 1. Sense电压瞬时(AD)值(每个计算周期清零)
	int32_t IoutIns[8];					// 2. 输出电流瞬时(AD)值
	int32_t VtzIns[8];						// 3. 探针电压瞬时(AD)值
	int32_t VsenseSum[8];					// 4. Sense电压累计(AD)值(每个计算周期不清零)
	int32_t VmodSum[8];					// 5. 端口电压累计(AD)值
	int32_t IoutSum[8];					// 6. 输出电流累计(AD)值
...
{
    IntelProtect_OLA = 0x01<<0,				// 1.输出限流保护 0x01
    IntelProtect_BOV = 0x01<<1,				// 2.BUS过压 0x02
    IntelProtect_BUV = 0x01<<2,				// 3.BUS欠压 0x04
    IntelProtect_APA = 0x01<<3,				// 4.电流精度告警 0x08
    IntelProtect_UPA = 0x01<<4,				// 5.电压精度告警 0x10
    IntelProtect_VDQ = 0x01<<5,				// 6.短路保护告警 0x20
    IntelProtect_MOT = 0x01<<6,				// 7.模块过温 0x40
    IntelProtect_EOT = 0x01<<7,				// 8.环境过温 0x80
    IntelProtect_HPA = 0x01<<8,				// 9.硬件保护 0x100
    IntelProtect_FHE = 0x01<<9,				// 10.风扇故障 0x200
    IntelProtect_UHE = 0x01<<10,			// 11.CPU异常 0x400
    IntelProtect_C1E = 0x01<<11,			// 12.can1异常 0x800
    IntelProtect_C2E = 0x01<<12,			// 13.can2异常 0x1000
    IntelProtect_DAE = 0x01<<13,			// 14.DA异常 0x2000
    IntelProtect_ADE = 0x01<<14,			// 15.AD异常 0x4000
    IntelProtect_FLE = 0x01<<15,			// 16.FLASH异常 0x8000
    IntelProtect_EEE = 0x01<<16,			// 17.Eeprom异常  0x10000
    IntelProtect_FRE = 0x01<<17,			// 18.FRAM异常 0x20000
    IntelProtect_PME = 0x01<<18,			// 19.电位器异常 0x40000
    IntelProtect_VNN = 0x01<<19,			// 20.电压异常 0x80000
    IntelProtect_CVP = 0x01<<20,			// 21.回路压差保护 0x100000
    IntelProtect_VUQ = 0x01<<21,			// 22.电压快速上升 0x200000
    IntelProtect_CNN = 0x01<<22,			// 23.电流异常 0x400000
    IntelProtect_CUQ = 0x01<<23,			// 24.电流快速上升 0x800000
    IntelProtect_LOV = 0x01<<24,			// 25.辅助电压异常（探针电压大于5.1V）  0x1000000   
    IntelProtect_MAC = 0x01<<25,      // 26.MAC异常（与条码不符）
    IntelProtect_CHE = 0x01<<26,			// 27.通道模块异常 
		IntelProtect_OSF = 0x01<<27,			// 28.输出短路 
		IntelProtect_BSF = 0x01<<28,			// 29.电池短路 
		IntelProtect_NEG = 0x01<<29,			// 30.反接保护 
		IntelProtect_CDE = 0x01<<30,			// 31.校准数据异常 
//		IntelProtect_SDE = 0x01<<31,			// 32.工步数据异常 
//		IntelProtect_OCP = 0x01<<32,			// 34.硬件OCP
//		IntelProtect_OVP = 0x01<<33,			// 35.硬件OVP
//		IntelProtect_OTP = 0x01<<34,			// 33.硬件OTP	
		IntelProtect_SDE = 0x20000000,			// 32.工步数据异常 
		IntelProtect_OCP = 0x40000000,			// 33.硬件OCP
		IntelProtect_OVP = 0x80000000,			// 34.硬件OVP
		IntelProtect_OTP = 0x100000000,			// 35.硬件OTP
		
//		IntelProtect_OTP = 0x01<<27,			// 28.硬件OTP
//		IntelProtect_OCP = 0x01<<28,			// 29.硬件OCP
//		IntelProtect_OVP = 0x01<<29,			// 30.硬件OVP 		
//		IntelProtect_NEG = 0x01<<30,			// 31.反接保护
};
...
typedef struct
{
   ...
   uint64_t P_Flg ;           //告警标志位
   uint8_t  Channel;          //当前告警通道
   uint8_t  LastChannel;      //上次告警通道
}CMDProject;

extern CMDProject IP_CheckList[35];
...
extern __IO float   data_upcast_period;
extern uint64_t IP_Alarm_Flag[8];
