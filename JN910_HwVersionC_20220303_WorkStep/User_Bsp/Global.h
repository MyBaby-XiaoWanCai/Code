...
{
	uint32_t  ChEn;                       /* 通道启停 */
	uint32_t DacSlopeTime;                /* 电流DA斜率-时间 */
	uint32_t DacSlopeStep;                /* 电流DA斜率-步长 */
	float    DacSlopeVmin;                /* 电流DA斜率-DA最小电压 */
	float    DacSlopeVmax;                /* 电流DA斜率-DA最大电压 */
	float    Tovcb;                       /* 控制板过温保护值 < cb   - control board > */
	float    Tovpb;                       /* 功率板过温保护值 < pb   - power board   > */
	float    Vcabt;                       /* 电池充电截止电压 < cabt - charge abort  > */
}s_DebugParamRwStruct;
...
typedef struct
{
	float Vsense[8];                        /* 电池电压 */
	float Vtz[8];                           /* 探针电压 */
	float Ibp[8];                           /* 旁路电流 < bp - by pass > */
	float Tcb;                              /* 控制板温度 */
	float Tpb[8];                           /* 功率板温度 */
}s_DebugParamRoStruct;

/* 调试参数结构体 */
typedef struct
{
	s_DebugParamRwStruct Rw;               /* 可读可写调试参数结构体 */
	s_DebugParamRoStruct Ro;               /* 只读调试参数结构体 */
}s_DebugParamStruct;
