...
void Err_Info_Pack(u8_t _no, u8_t* err_dat, u16_t* err_len, u32_t* _id)
{
	...
	*_id = (4 << 26) | (feat << 18) | (0 << 10) | (0 << 2) | (1 << 1) | (1 << 0);
}
...
extern uint8_t CAN2_SendData(uint32_t ID, uint8_t *pData, uint16_t len);
...
void CAN_SendMultiFrames(uint32_t id, uint8_t* pData, uint16_t len)
{
	...
	while(lenTemp)
	{
		...
		if(lenTemp > 8)
		{
			while(1 != CAN2_SendData(idTemp, p, 8));
			lenTemp -= 8;
			p       += 8;			
		}
		else
		{
			while(1 != CAN2_SendData(idTemp, p, lenTemp));
			lenTemp = 0;
		}
	}
}
