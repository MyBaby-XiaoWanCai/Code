...
CMDProject IP_CheckList[] =
{
    ...
    {0x1A, IntelProtect_MAC, 0, 0},			// MAC异常
		...
		{0x1B, IntelProtect_OTP, 0, 0},			// 硬件保护 - OTP
		{0x1C, IntelProtect_OCP, 0, 0},			// 硬件保护 - OCP
		{0x1D, IntelProtect_OVP, 0, 0},			// 硬件保护 - OVP
		{0x1E, IntelProtect_NEG, 0, 0},			// 硬件保护 - 反接
};
...
uint32_t IntelProtectDeviceCheck()
{
	...
	{	
		...
		{
//			bM4_RMU->RSTF0_b.CLRF = 0x01;   
			SET_REG32_BIT(M4_RMU->RSTF0,RMU_RSTF0_CLRF); //清除CLRF复位标志
			__NOP();
			__NOP();
			__NOP();
			__NOP();
		...
		{
			bM4_RMU->RSTF0_b.CLRF = 0x01;   //清除CLRF复位标志
			__NOP();
			__NOP();
			__NOP();
			__NOP();
