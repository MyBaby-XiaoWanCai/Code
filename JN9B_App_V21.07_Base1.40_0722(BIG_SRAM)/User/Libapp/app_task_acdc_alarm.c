...
enum tw_state
{
    TWFN_ER = 0x01 << 0,  //风扇故障
    TWMX_ER = 0x01 << 1,  //母线故障
    TWZL_ER = 0x01 << 2,  //整流器故障
    TWTO_ER = 0x01 << 3,  //过温故障
    TWNT_ER = 0x01 << 4,  //无温度传感器故障
    TWDC_OV = 0x01 << 5,  //DC过压
    TWDC_UV = 0x01 << 6,  //DC欠压
    TWDC_ST = 0x01 << 7,  //DC短路
    TWAC_ER = 0x01 << 8,  //AC故障
...
extern uint8_t TW_ABitFlg;          //图为告警变化位 =1标识有变化的bit
extern uint8_t TW_HisStatus;        //图为历史状态
extern uint8_t TW_SBitFlg;          //图为状态变化位 =1标识有变化的bit
extern uint8_t TW_AS_UpdateFlag;    //图为AS同步标志
...
void AppACDC_alarm(void *argument)
{
    ...
    uint8_t  tw_alarm;       //图为bit=1的且有变化的告警位
    uint8_t i;
//	int test_printf_acdc = 0;
    while(1)
    {
        if(osSemaphoreAcquire(acdc_alarm_Semaphore, osWaitForever) == osOK) //等待同步信号量
        {
            ...
            case 3:   //图为告警处理
                if(TW_AS_UpdateFlag&0x10)                 //告警变化处理
                {
                    for(i = 0; i < 8; i++)
                    {
                        ...
                        TW_AS_UpdateFlag&=~(0x10);            //清同步标志
                    }
                }
                else if(TW_AS_UpdateFlag&0x01)           //状态变化处理
                {
