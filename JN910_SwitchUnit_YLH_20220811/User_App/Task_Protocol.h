...
/* 版本号(V4.00A) */
#define SOFTWARE_PRIVERSION					4.00f	/* 主版本号 */
#define SOFTWARE_SUBVERSION					'A'		/* 子版本号 */
...
typedef struct
{
	uint8_t CHCtrEN;      	//通道控制使能
    uint8_t ByPassCtr;    	//强制控制旁路
    uint8_t MainRoadCtr;  	//强制控制主路
    uint8_t RoadSwitch;   	//旁路主路切换
    float DAMax;          	// DA输出最大值
    float DASet;          	// DA输出设定值
    uint16_t DAUpTime;    	// DA输出上升时间
    uint16_t DAHoldTime;  	// DA到达设定值保持时间
    uint16_t DADelayTime; 	// DA关断延时时间
    uint16_t DeadTime;    	//主路旁路切换死区时间
	float SetVsense;		//Vsense设置共模电压
	float SetVtz;        	//Vtz设置共模电压
