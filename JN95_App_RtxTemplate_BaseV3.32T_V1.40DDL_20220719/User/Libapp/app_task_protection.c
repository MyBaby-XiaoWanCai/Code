...
//						}
//					}
//					if(Tier2.sampleCal.Vbus2 < Tier2.iProt[i].VbusThrL.value)		//bus2对应后一半通道
//					{
//						ucIProtCntCur[11][i]++;
...
void AlarmQuarry(void)
{
    ...
    for(i = 0; i < CHANNEL_CFG; i++)
    {
        if(IP_Alarm_Flag[i] != 0)
        {
            ...
            lenth = 8 + cnt;
            AlarmSendBuf[2] = (lenth & 0x00ff) >> 0;	//更新帧长
...
void AppTaskProtection(void *argument)
{
    ...
    AlarmSendBuf[0] = 0x5B;		//下位机给中位机发，帧头0x5B5B
    AlarmSendBuf[1] = 0x5B;
    AlarmSendBuf[2] = 0x09;		//固定长度9
    AlarmSendBuf[3] = 0x00;
    AlarmSendBuf[4] = 0x00;		//特征码，每次上报自加1
