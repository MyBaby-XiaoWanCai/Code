...
{  
    ...
    static uint8_t FanReadNum = 0x06;
		static uint8_t ERR_Num[6]={0};	//各风扇错误累计数
		
    /* 读取风扇故障状态 */
    if(CHANNEL_USE == 0x08)	//8通道、120A
        FanReadNum = 0x06;
    else if(CHANNEL_USE == 0x04) //4通道、120A
        FanReadNum = 0x03;
    else if(CHANNEL_USE == 0x02) //2通道、120A
        FanReadNum = 0x02;		
    ...
    for(fan_i = 0; fan_i < FanReadNum; fan_i++)
    {
        ...
        {
            ERR_Num[fan_i]+=1;                            //风扇通道错误累计+1
        }
        else
        {
            ERR_Num[fan_i]=0;                             //风扇通道错误累计清0
            IOTestInfo.FanFault[fan_i] = 0;               //无故障
        }
        
        if(ERR_Num[fan_i]==10)                            //连续10次读取风扇故障，需要告警
        {
           IOTestInfo.FanFault[fan_i] = 1;                //故障
           IP_Alarm_Flag[fan_i] |= IntelProtect_FHE;      //置位报警标志
           ERR_Num[fan_i]=0;                              //风扇通道错误累计清0           
        }        
    }
}
