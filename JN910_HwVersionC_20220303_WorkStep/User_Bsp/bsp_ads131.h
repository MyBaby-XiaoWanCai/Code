...
** Project     :
** --------------------------------------------------------------------------------------------------------
** File name   : bsp_ads131.h
** Author      : Wang Liang
** Date        : 2022-01-10
** Description : 
**********************************************************************************************************/

#ifndef _BSP_ADS131_H_
#define _BSP_ADS131_H_
...
**                                            宏定义区
**********************************************************************************************************/
#define ADS131_FILTER_NUM   100.0
#define ADS131_REF_VOL      1.2
#define GAIN_VOL            0.22
#define GAIN_IMOD           0.0086
#define CHANNEL_COUNT (2)   // ADS131M02 -> 2 Channels

#if ((CHANNEL_COUNT < 1) || (CHANNEL_COUNT > 8))
    #error Invalid channel count configured in 'bsp_ADS131.h'.
#endif

#define NUM_REGISTERS                           ((uint8_t) 64)  //寄存器数量
...
typedef struct
{
	uint16_t AdsCollectCnt;        //ADS131M01循环采集次数
	uint16_t AdsCsFlag;            //下一片ADS131M01的CS使能标志
	uint32_t Vbat[8];
	uint32_t Vtz[8];
	uint32_t Imod[8];
}ADS131_DataType;
...
**                                         外部引用变量区
**********************************************************************************************************/	
extern adc_channel_data AdsData;
extern ADS131_DataType ADS131_Data;
