...
{
    uint8_t Rx_data = 0;
    HAL_SPI_TransmitReceive(&SPI6_Handler, &data, &Rx_data, 1, 0xff);
...
{
    uint8_t Rx_data = 0; 
    HAL_SPI_TransmitReceive_DMA(&SPI6_Handler,data, &Rx_data, Size);
    bsp_DelayUS(1); 
    return Rx_data;
}
...
{
    uint8_t Rx_data = 0; 
    HAL_SPI_Transmit_DMA(&SPI6_Handler, data, Size);
    bsp_DelayUS(Size); 
    return Rx_data;
}
/*
