...
//			uint8_t temp_arry[CHANNEL_CFG * 1024] = {0};
			...
			MB85Ctrl_Write(8192, &temp8_tt, 1 ,0);
			osDelay(5);
			...
			for(int i = 0; i < CHANNEL_CFG; i++)
			{
				if(write_flag_disable[i] == 1)
				{
					...
					MB85Ctrl_Write(addr_offset * i, temp_arry + addr_offset * i, addr_offset ,0);
			...
			for(int i = 0; i < CHANNEL_CFG; i++)
			{
				...
				{				
					...
					MB85Ctrl_Write(4096 + addr_offset * i, temp_arry + addr_offset * i, addr_offset ,0);
