...
uint8_t Txdata_E[1500];
uint8_t Rxdata_E[1500];
void sRead_SOCK_Data_Buffer(uint8_t ubSocket, uint8_t * Value,uint16_t Len)
{
    ...
    W5500_CS_0();
	  memset(Txdata_E, 0, Len+3);
		Txdata_E[0]=(uint8_t)(offset>>8);
		Txdata_E[1]=(uint8_t)offset;
		Txdata_E[2]=OM_N_Byte|RWB_READ|(ubSocket*0x20+0x18);
	
	  HAL_SPI_TransmitReceive(&hspi1, Txdata_E, Rxdata_E,3+Len,100*(3+Len));
    W5500_CS_1();
    Offset1 = offset + Len;
