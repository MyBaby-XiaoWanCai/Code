...
*	文件名称 : ads1256.c
*	版    本 : V1.0
*	说    明 : ADS1256模块和CPU之间采用SPI接口。本驱动程序支持软件SPI接口，硬件SPI。
*			  
*
*	修改记录 :
*		版本号  日期         作者     说明
...
*/
  ...
  #define PIN_DRDY             GPIO_PIN_6
	...
	#define DO_IS_HIGH()	(HAL_GPIO_ReadPin(PORT_DOUT, PIN_DOUT) == SET)

	#define DRDY_IS_LOW()	(HAL_GPIO_ReadPin(PORT_DRDY, PIN_DRDY) == RESET)
#endif
...
void InitADS1256(void)
{
	...
	GPIO_Initure.Pin = PIN_SCK | PIN_DIN | PIN_CS | PIN_RESET; /*输出引脚 */
	...
	GPIO_Initure.Pin = PIN_DOUT; /*输入引脚 */
	...
	GPIO_Initure.Pin =  PIN_CS | PIN_RESET; /*输出引脚 */
	GPIO_Initure.Mode = GPIO_MODE_OUTPUT_PP; /* 设为推挽模式 */
	GPIO_Initure.Pull = GPIO_NOPULL;         /* 上下拉电阻不使能 */
	GPIO_Initure.Speed = GPIO_SPEED_HIGH;    /* IO口最大速度 */
	HAL_GPIO_Init(GPIOF, &GPIO_Initure);     //GPIOF
...
static void ADS1256_Send8Bit(uint8_t _data)
{
	...
	#endif
	#ifdef HARD_SPI		
	uint8_t Txdata[1] = {_data};
  uint8_t Rxdata[1] = {0X00};
	HAL_SPI_TransmitReceive(&hspi6, Txdata, Rxdata, 1,100);
	#endif
}
