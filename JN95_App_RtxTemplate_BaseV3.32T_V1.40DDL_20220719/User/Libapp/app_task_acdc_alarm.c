...
extern uint8_t AACV_FLAG;                    //AC故障电压更新标志
extern uint8_t ADCV_FLAG;                    //AC故障电压更新标志
/*国电报警状态*/
enum gd_state
{
    ...
    while(1)
    {
        if(osSemaphoreAcquire(acdc_alarm_Semaphore, osWaitForever) == osOK) //等待同步信号量
        {
            ...
            case 2:   //普德报警处理
                ...
                if(((ACDC.State[0] & PDAC_OV)||(ACDC.State[0] & PDAC_UV))&&(AACV_FLAG==1)) //模块0 AC过压或欠压时判断
                {                                        
                   memset(str,0,sizeof(str)); 
                   sprintf(str,"模块0故障AC电压=%1f\r\n",ACDC.AACV[0]);
                   W5500_send(0, (uint8_t *)str, 13);    //网口发送模块0故障AC电压 
                   AACV_FLAG=0;                    
                }

                if(((ACDC.State[1] & PDAC_OV)||(ACDC.State[1] & PDAC_UV))&&(AACV_FLAG==1)) //模块1 AC过压或欠压时判断
                {   
                   memset(str,0,sizeof(str)); 
                   sprintf(str,"模块1故障AC电压=%1f\r\n",ACDC.AACV[1]);
                   W5500_send(0, (uint8_t *)str, 13);    //网口发送模块1故障AC电压
                   AACV_FLAG=0;                     
                }

                 if((ACDC.State[0] & PDDC_OV)&&(ADCV_FLAG==1))                            //模块0 DC过压时判断
                {   
                   memset(str,0,sizeof(str)); 
                   sprintf(str,"模块0故障DC电压=%1f\r\n",ACDC.ADCV[0]);
                   W5500_send(0, (uint8_t *)str, 13);    //网口发送模块0故障DC电压
                   ADCV_FLAG=0;                    
                }

                if((ACDC.State[1] & PDDC_OV)&&(ADCV_FLAG==1))                             //模块1 DC过压时判断
                {   
                   memset(str,0,sizeof(str)); 
                   sprintf(str,"模块1故障DC电压=%1f\r\n",ACDC.ADCV[1]);
                   W5500_send(0, (uint8_t *)str, 13);    //网口发送模块1故障DC电压
                   ADCV_FLAG=0;                    
                }
                break;
            default :
