...
//**                                         全局变量定义区
//**********************************************************************************************************/
//uint8_t EE_WR_Flg = 0x00;	//EE读写标志 1 - 读写中，此时不读主控板TMP75的数据 0 - 读写操作结束
...
uint8_t ee_ReadBytes(uint8_t *_pReadBuf, uint16_t _usAddress, uint16_t _usSize)
{
    ...
    uint16_t t;
		for(t = 0; t < _usSize; t++)
		{
			*(_pReadBuf+t) = ee_ReadByte(_usAddress + t);
		}
...
uint8_t ee_ReadBytes2(uint8_t *_pReadBuf, uint16_t _usAddress, uint16_t _usSize)
{
		EE_WR_Flg = 0x01;
...
uint8_t ee_ReadBytes_wc2(uint8_t *_pReadBuf, uint16_t _usAddress, uint16_t _usSize)
{
    uint16_t i;
...
**             1 - 写入成功
**********************************************************************************************************/
void ee_WriteByte(uint16_t WriteAddr,uint8_t DataToWrite)
{
    i2c_Start(I2C_EE_TEMP);
    i2c_SendByte(I2C_EE_TEMP, EE_DEV_ADDR | I2C_WR);
		if(EE_ADDR_BYTES == 1)
    {
			i2c_SendByte(I2C_EE_TEMP, (uint8_t)WriteAddr);	
		}
		else
		{
			i2c_SendByte(I2C_EE_TEMP, WriteAddr >> 8);
	    i2c_SendByte(I2C_EE_TEMP, WriteAddr);
    }
    i2c_SendByte(I2C_EE_TEMP, DataToWrite);
    i2c_Stop(I2C_EE_TEMP);
    bsp_DelayUS(5000);
}
...
uint8_t ee_WriteBytes(uint8_t *_pWriteBuf, uint16_t _usAddress, uint16_t _usSize)
{
   ...
   for(t = 0; t < _usSize; t++) 
   {
     ee_WriteByte(_usAddress+t, *(_pWriteBuf+t));
...
uint8_t ee_WriteBytes2(uint8_t *_pWriteBuf, uint16_t _usAddress, uint16_t _usSize)
{
    uint16_t i, m;
    uint16_t usAddr;
